...
def get_from_github(device):
        print("Going to fetch %s from COLT github" % device)
        try:
        ...
        githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:ColtOS+in:name+fork:true" % device)
        if githubauth:
        ...
        for res in result['items']:
            if (res['name'].startswith("device_") and res['name'].endswith("_%s" % device)):
                ...
                devicetree = res['full_name'].replace("ColtOS/", "")
                if add_to_local_manifest(devicepath, devicetree, "colt"):
                    reposync(devicetree)
                break

def checkdeps(repo_path):
    coltdeps = glob.glob(repo_path + "/colt.dependencies")
    if ((len(coltdeps)) < 1):
        ran_checkdeps_on.append("NO_DEPS:\t\t" + repo_path)
        return
    else:
        if (len(coltdeps) > 0):
            ran_checkdeps_on.append("HAS_COLT_DEPS:\t" + repo_path)
            coltdeps = coltdeps[0]
            coltdeps = open(coltdeps, 'r')
            coltdeps = json.loads(coltdeps.read())
            for dep in coltdeps:
                try:
                    branch = dep['branch']
                except:

...
function addcompletions()
{
    ...
    dirs="sdk/bash_completion vendor/colt/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the colt model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch colt_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
	...
	local device=$(echo -n $product | sed -e "s/.*colt_//")
